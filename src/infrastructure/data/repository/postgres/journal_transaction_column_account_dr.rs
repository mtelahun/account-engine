    ...
    > for PostgresStore
{
    async fn insert(
        ...
        let sql = format!(
            ...
            journal::transaction::column::account_dr::ActiveModel::NAME
        );
        let res = conn
            .query_one(
                sql.as_str(),
        ...
        Ok(journal::transaction::column::account_dr::ActiveModel::from(
            res,
        ))
    }
    ...
    async fn get(
        ...
        let mut records = Vec::<journal::transaction::column::account_dr::ActiveModel>::new();
        for row in rows {
            let am = journal::transaction::column::account_dr::ActiveModel::from(row);
            records.push(am);
        }
    ...
    async fn save(
        &self,
        _model: &journal::transaction::column::account_dr::ActiveModel,
    ) -> Result<u64, OrmError> {
        todo!()
    }
...
impl From<Row> for journal::transaction::column::account_dr::ActiveModel {
