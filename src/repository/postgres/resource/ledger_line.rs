...
use crate::{
    ...
    entity::{ledger_line, LedgerKey},
    orm::{OrmError, Resource, ResourceOperations},
    repository::postgres::repository::PostgresRepository,
};

#[async_trait]
impl ResourceOperations<ledger_line::Model, ledger_line::ActiveModel, LedgerKey>
    for PostgresRepository
{
    ...
    async fn search(&self, _domain: &str) -> Result<Vec<ledger_line::ActiveModel>, OrmError> {
        todo!()
    }

    async fn save(&self, _model: &ledger_line::ActiveModel) -> Result<u64, OrmError> {
        todo!()
    }
