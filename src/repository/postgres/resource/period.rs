...
use crate::{
    domain::ids::PeriodId,
    entity::{accounting_period, InterimType},
    orm::{period::AccountingPeriodService, OrmError, Resource, ResourceOperations},
...
#[async_trait]
impl AccountingPeriodService for PostgresRepository {
    async fn create(
        &self,
        model: &accounting_period::Model,
    ) -> Result<accounting_period::ActiveModel, OrmError> {
        ...
        if periods.is_empty() {
            let active_model = self.insert(model).await?;
